        ...
        if (IS_POST) {
            ...
            if (self::$user->login($username, $password, isset($_POST['remember']))) {
                ...
                model('Users')->recalculate_price();
                    ...
                    $openid=$_COOKIE['ECS']['openid'];
                //防止一个账号登陆两个微信绑定混乱
                $u_row = model('Base')->model->table('wechat_user')
                    ->field('nickname, headimgurl,localheadimgurl')
                ...
                //用户关系绑定
                    if (isset($openid) && !empty($openid)) {
                        $sql = 'select sceneid from ' . M()->pre . 'wechat_user' . "  where `openid`='" . $openid . "'";
                        $wechat = M()->query($sql);
                        foreach ($wechat AS $val)
                        {
                            $parent_id =$val['sceneid'] ;
                        }
                        $sql = 'UPDATE ' . M()->pre . 'wechat_user' . " SET `ect_uid`=' $user_id' WHERE `openid`='" . $openid . "'";
                        M()->query($sql);
                        if (isset($parent_id) && !empty($parent_id)) {
                            $sql = 'UPDATE ' . M()->pre . 'users' . " SET `parent_id`='$parent_id' WHERE `user_id`='" . $_SESSION['user_id'] . "'";
                            M()->query($sql);
                        }
                        //微信端登录后关注积分和注册积分合并
                        $sql = 'select scenepoint from ' . M()->pre . 'wechat_user' . "  where `openid`='" . $openid . "'";
                        $wechat = M()->query($sql);
                        foreach ($wechat AS $val)
                        {
                            $scenepoint =$val['scenepoint'] ;
                        }
                        // 判断是否有关注积分
                        if (empty($scenepoint)) {
                        ...
                        {
                            //增加用户积分
                            $point = 'rank_points = rank_points +' . intval($scenepoint);
                            ...
                            //清空关注积分
                            $point_value=0;
                            $sql = 'UPDATE ' . M()->pre . 'wechat_user' . " SET `scenepoint`=' $point_value' WHERE `openid`='" . $openid . "'";
                            M()->query($sql);
                        }
                    }
                }
        ...
        if (IS_POST) {
            ...
            elseif (1 == $enabled_sms) {
				...
				if ($password != $comfirm_password) {
					show_message(L('password_equal_error'), L('password_equal_error'), url('register'), 'error');
				}
            ...
            if (model('Users')->register($username, $password, $email, $other , C('send_type_rand') !== false) !== false) {
                ...
                //                查出user_id,并将其赋值给wechat_user中的ect_ui
                $user_id= $_SESSION['user_id'];
                $openid=$_COOKIE['ECS']['openid'];
                //注册送积分
                $point_value=5;
