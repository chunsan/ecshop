    ...
    public function index()
    {
        ...
        if ($type == Wechat::MSGTYPE_TEXT) {
            ...
            if ('subscribe' == $wedata['Event']) {
                ...
                if ($keywords == 'yqhy') {
                    $handler = new ShareButton();
                    $openid= $wedata['FromUserName'];
                    $sql = 'select ect_uid from ' . M()->pre . 'wechat_user' . "  where `openid`='" . $openid . "'";
                    $wechat = M()->query($sql);
                    ...
                    $wedata['scene_id']=$ect_uid;
                    $access_token = $this->weObj->checkAuth();
                    $wedata['access_token']=$access_token;
                    $handler->handle($wedata);
                }
...
//                    $info['ect_uid'] = $ect_uid;
                ...
                $info['subscribe'] = 1;
                $info['openid'] = $openid;
                if (isset($info['sceneid']) && !empty($info['sceneid'])){
                }else{
                    $info['sceneid'] =$scene_id;//推荐人ID
                }
                //对用户头像进行处理
                $headimgurl = $info["headimgurl"];
                if (isset($headimgurl) && !empty($headimgurl))
                {$file = downloadfile($headimgurl);
                    $copy_dir = './themes/yemaijiu/images/';
                    if (!file_exists($copy_dir)) {
                        mkdir($copy_dir);
                    }
                    //将微信头像处理成适合手机显示的大小
                    $img_name = $this->imageSizeFormate($file, 70, $copy_dir);
                    if (isset($img_name) && !empty($img_name)) {
                        $info["localheadimgurl"] =$img_name . ".jpg";
                    }
                }else
                {
                }
                ...
                $msg = array(
                    ...
                    'msgtype' => 'text',
                    'text' => array(
                        'content' =>$info['nickname']. ',欢迎您关注众义微店！！！！'
                    )
                );
                if($msg['text']){
                    $this->weObj->sendCustomMessage($msg);
...
//                    if ($token) {
//                        logResult(var_export('openid='.$token['openid'],true));
